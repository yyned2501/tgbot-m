...
@app.on_message(filters.chat(BOT)&filters.regex(r"庄：\?\?\? ((?:[2-9JQKA][♦♣♥♠]\s*)+)\n你(\d+)点：((?:[2-9JQKA][♦♣♥♠]\s*)+)"))
async def blackjack(client: Client, message: Message):
    zhuang = message.matches[0].group(1)
    ni_dianshu = message.matches[0].group(2)
    ni = message.matches[0].group(3)
    ...
    await message.reply_text(f"庄的点数: {zhuang_score}\n你的点数: {ni_score}\n你的获胜概率: {win_probability:.2%}")
